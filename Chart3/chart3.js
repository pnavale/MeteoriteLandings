function dashboard(id, fData){
    ...
    function segColor(c){ return {fell:"#f03b20", found:"#feb24c"}[c]; }
    
    // calculate total freq for each country
    fData.forEach(function(d){d.total=d.freq.fell+d.freq.found;});
    
    // histogram function
    function histoGram(fD){
        var hG={};
        var hGDim = {t: 20, r: 0, b: 70, l: 50};
        ...
        function mouseover(d){
            ...
            var ct = fData.filter(function(s){ return s.country == d[0];})[0],
                nD = d3.keys(ct.freq).map(function(s){ return {type:s, freq:ct.freq[s]};});
    ...
    // calculate total frequency by segment for all countries.
    var tF = ['fell','found'].map(function(d){ 
